/* =====================================================================
   Marqques Barbershop — motor de agendamento
...
(function () {
  ...
  var CHAVE = 'mq_agendamentos_demo';
  ...
  function demoBloqueios() {
    try { return JSON.parse(localStorage.getItem('mq_bloqueios_demo') || '[]'); }
    catch (e) { return []; }
  }
  ...
  function demoSemeia() {
    try { if (localStorage.getItem('mq_demo_semeado_v2')) return; } catch (e) { return; }
    ...
    for (var d = 0; d < 8; d++) {
      ...
      CFG.barbeirosDemo.forEach(function (b) {
        var qtd = 2 + sorte(4);
        for (var i = 0; i < qtd; i++) {
          var s = CFG.servicosDemo[sorte(6)];
    ...
    demoGrava(lista);
    try { localStorage.setItem('mq_demo_semeado_v2', '1'); } catch (e) {}
  }
  ...
  try {
    var salvo = JSON.parse(localStorage.getItem('mq_cliente') || 'null');
  ...
  form.addEventListener('submit', function (ev) {
    ...
    API.criar({
      ...
      try { localStorage.setItem('mq_cliente', JSON.stringify({ nome: nome, tel: campoTel.value })); } catch (e) {}
  ...
  function mostraSucesso(a, nome) {
    ...
    var texto =
      'Olá, Marqques! Marquei pelo site.\n\n' +
  ...
  function fazIcs(a) {
    ...
    var linhas = [
      'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Marqques Barbershop//PT-BR',
      'BEGIN:VEVENT',
      'UID:' + a.codigo + '@marqquesbarbershop',
      'DTSTART;TZID=America/Bahia:' + carimbo(a.dia, a.inicio),
      'DTEND;TZID=America/Bahia:'   + carimbo(a.dia, a.fim),
      'SUMMARY:' + a.servico + ' com ' + a.barbeiro + ' — Marqques Barbershop',
